...
class TrackedSiteListView(FormMixin, ListView, ProcessFormView):
    ...
    def get_queryset(self):
        ...
        # Annoter le queryset avec le nombre total de pages et le nombre de pages indexées (sending)
        queryset = TrackedSite.objects.annotate(
            total_pages=Count('pages'),
            sending_pages=Count('pages', filter=models.Q(pages__status=PAGE_STATUS_INDEXED))
        )
